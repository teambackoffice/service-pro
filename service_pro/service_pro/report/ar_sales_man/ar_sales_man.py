...
class ReceivablePayableReport(object):
	...
	def get_invoice_details(self):
		...
		if self.party_type == "Customer":
			si_list = frappe.db.sql("""
				select name, due_date, sales_man_name
	...
	def get_columns(self):
		...
		self.add_column(label=_(self.party_type), fieldname='party',
			fieldtype='Link', options=self.party_type, width=130)

		if self.party_naming_by == "Naming Series":
			self.add_column(_('{0} Name').format(self.party_type),
				fieldname = scrub(self.party_type) + '_name', fieldtype='Data', width=300)

		self.add_column(label=_('Sales Man'), fieldname='sales_man_name', fieldtype='Data', width=180)
