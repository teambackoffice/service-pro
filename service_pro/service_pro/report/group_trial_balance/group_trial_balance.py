...
def get_data(filters):
	...
	accounts = frappe.db.sql(
		...
		from `tabAccount` where company in (select name from `tabCompany` where parent_company = %(company)s or name = %(company)s) order by lft""",
	...
	min_lft, max_rgt = frappe.db.sql(
		...
		where company in (select name from `tabCompany` where parent_company = %(company)s or name = %(company)s)""",
		{'company':filters.company},
	)[0]
...
def rf_set_gl_entries_by_account(
	company,
	from_date,
	to_date,
	root_lft,
	...
	additional_conditions = rf_get_additional_conditions(from_date, ignore_closing_entries, filters)

	accounts = frappe.db.sql_list(
		"""select name from `tabAccount`
		where lft >= %(lft)s and rgt <= %(rgt)s and company in (select name from `tabCompany` where name = %(company)s or parent_company = %(company)s) """,
		{'lft':root_lft,'rgt':root_rgt,'company':company}
	)
	...
	if accounts:
		...
		gl_entries = frappe.db.sql(
			"""
			select posting_date, account,ac.account_name, debit, credit, is_opening, fiscal_year,
				debit_in_account_currency, credit_in_account_currency, gl.account_currency from `tabGL Entry` gl, `tabAccount` ac
			where gl.account = ac.name
			and gl.company in (select name from `tabCompany` where parent_company = %(company)s or name = %(company)s)
...
def rf_get_additional_conditions(from_date, ignore_closing_entries, filters):
	additional_conditions = []
...
def get_rootwise_opening_balances(filters, report_type):
	...
	gle = frappe.db.sql(
		...
		from `tabGL Entry`
		where
			company in (select name from `tabCompany` where parent_company = %(company)s or name = %(company)s)
			{additional_conditions}
