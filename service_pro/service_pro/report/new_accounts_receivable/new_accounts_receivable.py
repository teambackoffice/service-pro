...
class ReceivablePayableReport(object):
	...
	def set_ageing(self, row):
		...
		row.total_due = row.range1 + row.range2 + row.range3 + row.range4 + row.range5 + row.range6
	...
	def get_ageing_data(self, entry_date, row):
		...
		if not (
			self.filters.range1 and self.filters.range2 and self.filters.range3 and self.filters.range4 and self.filters.range5
		):
			self.filters.range1, self.filters.range2, self.filters.range3, self.filters.range4, self.filters.range5= (
				30,
				60,
				90,
				120,
	...
	def setup_ageing_columns(self):
		...
		for i, label in enumerate(
			[
				...
				"{range5}-{above}".format(range5=cint(self.filters["range5"]) + 1, above=_("Above")),
			]
		):
