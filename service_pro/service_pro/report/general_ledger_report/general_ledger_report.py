...
# Modify the execute function to handle default company currency
...
# Modify set_account_currency function to handle when company isn't specified
def set_account_currency(filters):
    # If company is not specified, use default company for currency
    if not filters.get("company"):
        filters["company_currency"] = get_company_currency(get_default_company())
    else:
    ...
    if filters.get("account") or (filters.get("party") and len(filters.party) == 1):
        if filters.get("account"):
            ...
            # For party queries, we still need company to filter GL entries
            company_condition = f"company = '{filters.company}'" if filters.get("company") else ""
            
            gle_query = f"""
                SELECT account_currency 
                FROM `tabGL Entry`
                WHERE party_type = '{filters.party_type}'
                AND party = '{filters.party[0]}'
                {' AND ' + company_condition if company_condition else ''}
                LIMIT 1
            """
            
            gle_currency = frappe.db.sql(gle_query, as_dict=0)
            if gle_currency and gle_currency[0][0]:
                account_currency = gle_currency[0][0]
            else:
                account_currency = (
                    None
...
def get_gl_entries(filters, accounting_dimensions):
    ...
    # Build the WHERE clause based on whether company is specified
    company_filter = "company=%(company)s" if filters.get("company") else "1=1"
    ...
    gl_entries = frappe.db.sql(
        ...
        from `tabGL Entry`
        where {company_filter} {get_conditions(filters)}
        {order_by_statement}
    """,
        filters,
        as_dict=1,
...
# Modify the part where err_journals and system_generated_cr_dr_journals are queried
def get_conditions(filters):
    ...
    if filters.get("ignore_err") and filters.get("company"):
    ...
    if filters.get("ignore_cr_dr_notes") and filters.get("company"):
