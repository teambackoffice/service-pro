...
def get_gl_entries(filters, accounting_dimensions):
	...
	gl_entries = frappe.db.sql(
		"""
		select
			name as gl_entry,company, posting_date, account, party_type, party,
			voucher_type, voucher_no, {dimension_fields}
			cost_center, project,
			against_voucher_type, against_voucher, account_currency,
			remarks, against, is_opening, creation {select_fields}
		from `tabGL Entry`
		where company in (select name from `tabCompany` where parent_company = %(company)s or name = %(company)s) {conditions}
		{order_by_statement}
	""".format(
...
def get_conditions(filters):
	...
	if filters.get("account"):
		...
		conditions.append("account in (select name from `tabAccount` where company in (select name from `tabCompany` where parent_company = %(company)s or name = %(company)s) AND account_name in (select account_name from tabAccount where name in %(account)s))")
